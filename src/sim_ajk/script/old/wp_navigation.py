...
class WpNavi():
  ...
  def run(self):
    ...
    way_point = [[self.robot_pose[0], self.robot_pose[1], self.robot_yaw]]
    for i in range(Straight_Split):
      Splited_Dist = Straight_Dist/Straight_Split * (i + 1)
      x = np.cos(-self.robot_yaw) * Splited_Dist + self.robot_pose[0]
      y = -np.sin(-self.robot_yaw) * Splited_Dist + self.robot_pose[1]
      way_point.append([x, y, self.robot_yaw])

    print np.round(way_point, 2)
    f = open('text.txt', 'w')
    for i in way_point:
      f.write(str(i) + "\n")
    f.close()
